...
public class PartyLedger extends AppCompatActivity {
    String usertype,username,billnumber,open="",salesman="";
    TextView txtamount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try
        {
            ...
            {
                Toast.makeText(PartyLedger.this, "No Internte", Toast.LENGTH_SHORT).show();
            }
            atvparty.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    if (isNetworkAvailable()) {
                        Intent intent = new Intent(getApplicationContext(), LedgerDetails.class);
                        String partyname="";
                        ...
                        {
                            Vector v=(Vector)ledger.elementAt(i);
                            if(v.elementAt(0).toString().equals(partyname))
                            {
                                open=""+v.elementAt(2).toString().trim();
                            }
                        }
                        intent.putExtra("Opening",""+open);
    ...
    private void loadledgerDetails()
    {
        try
        {
            ...
            for(int i=0;i<ledger.size();i++)
            {
                ...
                try {
                    tot = Double.parseDouble("" + data.elementAt(1));
                ...
                if(tot<0)
                {
                    total[i]="Dr  "+getResources().getString(R.string.Rs)+" "+roundTwo(Math.abs(tot));
                }
                else
                {
                    total[i]="Cr  "+getResources().getString(R.string.Rs)+" "+roundTwo(tot);
                }
            ...
            txtamount.setText(""+getResources().getString(R.string.Rs)+" "+Math.abs(roundTwo(totalledger)));
    ...
    private class LoadLedgerCount extends AsyncTask<String, Void, Void> {
        ...
        private ProgressDialog Dialog = new ProgressDialog(PartyLedger.this);
        ...
        protected Void doInBackground(String... urls) {
            try {
                ...
                try {
                    ...
                    HttpPost httppost = new HttpPost(new JsonBilder().getHostName() + "selectledgersalesman.php");
            ...
            if (Error != null) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++)
                    {
                        ...
                        o.addElement(jsonObject.getString("Closing"));
                        ledger.addElement(o);
                        //Log.e("",""+o.toString());
                        arrlst.add(jsonObject.getString("Party_Name"));
                    }
    ...
    private boolean isNetworkAvailable() {
        int permissionCheck = ContextCompat.checkSelfPermission(PartyLedger.this, Manifest.permission.INTERNET);

        if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(PartyLedger.this, new String[]{Manifest.permission.INTERNET}, INTERNET);
